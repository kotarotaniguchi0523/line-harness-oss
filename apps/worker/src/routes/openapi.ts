		...
		"/api/friends/{id}/tags": {
			post: {
				...
				requestBody: {
					content: {
						"application/json": {
							schema: { type: "object", properties: { tagId: { type: "string" } }, required: ["tagId"] },
						},
					},
				},
		...
		"/api/users/match": {
			post: {
				...
				requestBody: {
					content: {
						"application/json": {
							schema: { type: "object", properties: { email: { type: "string" }, phone: { type: "string" } } },
						},
					},
				},
		...
		"/api/users/{id}/link": {
			post: {
				...
				requestBody: {
					content: {
						"application/json": {
							schema: { type: "object", properties: { friendId: { type: "string" } }, required: ["friendId"] },
						},
					},
				},
				...
				requestBody: {
					content: {
						"application/json": {
							schema: {
								type: "object",
								properties: { name: { type: "string" }, eventType: { type: "string" }, value: { type: "number" } },
				...
				requestBody: {
					content: {
						"application/json": {
							schema: {
								type: "object",
								properties: { name: { type: "string" }, code: { type: "string" }, commissionRate: { type: "number" } },
								required: ["name", "code"],
							},
						},
					},
...
// GET /openapi.json - raw spec (cached for staticCacheMaxAge)
openapi.get("/openapi.json", (c) => {
	c.header("Cache-Control", `public, max-age=${MIDDLEWARE_LIMITS.staticCacheMaxAge}`);
	return c.json(spec);
});

// GET /docs - Swagger UI (cached for staticCacheMaxAge)
openapi.get("/docs", (c) => {
	c.header("Cache-Control", `public, max-age=${MIDDLEWARE_LIMITS.staticCacheMaxAge}`);
