...
export function registerManageAdPlatforms(server: McpServer): void {
	server.tool(
		...
		{
			action: z.enum(["list", "create", "update", "delete", "test"]).describe("Action to perform"),
			platformId: z.string().optional().describe("Platform ID (required for 'update' and 'delete')"),
			name: z
				.enum(["meta", "x", "google", "tiktok"])
				.optional()
				.describe("Platform name (required for 'create' and 'test')"),
			displayName: z.string().optional().describe("Display name for the platform (e.g. 'Meta広告')"),
			config: z
				.record(z.unknown())
				.optional()
				.describe(
			...
			isActive: z.boolean().optional().describe("Enable/disable the platform (for 'update')"),
			eventName: z.string().optional().describe("Event name for test conversion (for 'test', e.g. 'Lead')"),
			friendId: z.string().optional().describe("Friend ID for test conversion (for 'test')"),
		},
		async ({ action, platformId, name, displayName, config, isActive, eventName, friendId }) => {
			try {
				...
				switch (action) {
					...
					case "update": {
						if (!platformId) throw new Error("platformId is required for update action");
					...
					case "delete": {
						if (!platformId) throw new Error("platformId is required for delete action");
					...
					case "test": {
						...
						if (!eventName) throw new Error("eventName is required for test action");

						const result = await client.adPlatforms.test(name, eventName, friendId);
