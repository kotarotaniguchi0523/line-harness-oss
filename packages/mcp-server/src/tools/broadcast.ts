	...
	server.tool(
		...
		"Send a broadcast message to all friends, a specific tag group, or a filtered segment. Creates and immediately sends the broadcast.",
		{
			...
			targetType: z
				...
				.default("all")
				.describe("Target audience: 'all' for everyone, 'tag' for a tag group, 'segment' for filtered conditions"),
		...
		async ({
			...
			try {
				...
				if (targetType === "segment" && segmentConditions) {
					...
					const { content: trackedContent } = await autoTrackUrls(client, messageContent, messageType, title);
					...
					try {
						...
						return {
							content: [
								{
									type: "text" as const,
									text: JSON.stringify({ success: true, broadcast: result }, null, 2),
								},
							],
						};
					} catch (sendError) {
				...
				const { content: trackedContent, trackedUrls: _trackedUrls } = await autoTrackUrls(
					client,
					messageContent,
					messageType,
					title,
