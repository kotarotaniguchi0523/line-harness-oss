	...
	server.tool(
		...
		"Create a step delivery scenario with multiple message steps. Each step has a delay and message content. Scenarios auto-trigger on friend_add, tag_added, or manual enrollment.",
		{
			...
			triggerTagId: z
				.string()
				.optional()
				.describe("Required when triggerType is 'tag_added': the tag ID that triggers this scenario"),
			steps: z
				.array(
					z.object({
						delay: z
							.string()
							.describe("Delay before sending. Format: '0m' for immediate, '30m' for 30 minutes, '24h' for 24 hours"),
