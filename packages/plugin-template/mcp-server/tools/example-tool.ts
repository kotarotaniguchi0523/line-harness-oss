...
export function registerExampleTool(server: McpServer): void {
			...
			try {
				...
				if (customer.lineUserId) {
					...
					while (!lineFriend) {
						...
						lineFriend = page.items.find((f) => f.lineUserId === customer.lineUserId) ?? null;
	...
	server.tool(
		...
		"Send a MyService-related notification (appointment reminder, etc.) to a LINE friend.",
		{
			...
			message: z.string().optional().describe('Custom message text (required for "custom" type)'),
			customerId: z.string().optional().describe("MyService customer ID (for auto-generating message content)"),
		},
