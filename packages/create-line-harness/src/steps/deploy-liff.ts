...
export async function deployLiff(options: DeployLiffOptions): Promise<DeployLiffResult> {
	const s = p.spinner();
	...
	try {
		...
		s.stop("LIFF ビルド失敗");
		throw new Error(`LIFF のビルドに失敗しました: ${error instanceof Error ? error.message : error}`);
	}
	s.stop("LIFF ビルド完了");
	...
	try {
		...
		s.stop("LIFF デプロイ失敗");
		throw new Error(`LIFF のデプロイに失敗しました: ${error instanceof Error ? error.message : error}`);
	}
}
