...
export async function deployAdmin(options: DeployAdminOptions): Promise<DeployAdminResult> {
	const s = p.spinner();
	...
	try {
		...
		throw new Error(`Admin UI のビルドに失敗しました: ${error instanceof Error ? error.message : error}`);
	}
	...
	try {
		...
		try {
			...
			const subdomainMatch = projectList.match(new RegExp(`${options.projectName}\\s+│\\s+(\\S+\\.pages\\.dev)`));
			if (subdomainMatch) {
		...
		throw new Error(`Admin UI のデプロイに失敗しました: ${error instanceof Error ? error.message : error}`);
	}
}
