...
const ACCOUNT_ID_RE = /│\s+\S.*?\s+│\s+([a-f0-9]{32})\s+│/;
...
export async function getAccountId(): Promise<string> {
	...
	const match = output.match(ACCOUNT_ID_RE);
	if (!match) {
		throw new Error("Cloudflare アカウント ID を取得できません。wrangler whoami の出力を確認してください。");
	}
	return match[1];
}
