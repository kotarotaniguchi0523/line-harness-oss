...
import { WranglerError, wrangler } from "../lib/wrangler.js";

const DATABASE_ID_TOML_RE = /database_id\s*=\s*"([^"]+)"/;
const DATABASE_ID_JSON_RE = /"database_id"\s*:\s*"([^"]+)"/;
...
export async function createDatabase(repoDir: string): Promise<DatabaseResult> {
	const s = p.spinner();
	...
	try {
		...
		const tomlMatch = output.match(DATABASE_ID_TOML_RE);
		const jsonMatch = output.match(DATABASE_ID_JSON_RE);
		const uuidMatch = output.match(UUID_RE);
	...
	try {
		await wrangler(["d1", "execute", databaseName, "--remote", "--file", schemaFile]);
	} catch {
	...
	for (const file of migrationFiles) {
		try {
			await wrangler(["d1", "execute", databaseName, "--remote", "--file", join(migrationsDir, file)]);
		} catch {
