...
/** Safely retrieve a required state field, throwing if missing. */
function requireState<K extends keyof SetupState>(state: SetupState, key: K): NonNullable<SetupState[K]> {
	const value = state[key];
	if (value == null) {
		throw new Error(`Setup state "${key}" is required but missing. Please re-run setup.`);
	}
	return value as NonNullable<SetupState[K]>;
}
...
async function promptLiffId(): Promise<string> {
	p.log.message(
		[
	...
	return (liffId as string).trim();
}

async function registerLineAccount(state: SetupState): Promise<void> {
	const s = p.spinner();
...
export async function runSetup(repoDir: string): Promise<void> {
	...
	if (state.completedSteps.length > 0) {
		p.log.info(`前回の途中から再開します（完了済み: ${state.completedSteps.join(", ")}）`);
	}
	...
	if (!isDone(state, "liffId")) {
		state.liffId = await promptLiffId();
	...
	if (!isDone(state, "worker")) {
		const { workerUrl } = await deployWorker({
			repoDir,
			d1DatabaseId: requireState(state, "d1DatabaseId"),
			d1DatabaseName: requireState(state, "d1DatabaseName"),
			workerName,
			accountId: requireState(state, "accountId"),
		});
	...
	if (!isDone(state, "secrets")) {
		await setSecrets({
			workerName,
			lineChannelAccessToken: requireState(state, "lineChannelAccessToken"),
			lineChannelSecret: requireState(state, "lineChannelSecret"),
			lineLoginChannelId: requireState(state, "lineLoginChannelId"),
			apiKey: requireState(state, "apiKey"),
		});
	...
	if (!isDone(state, "lineAccount")) {
		await registerLineAccount(state);
	...
	if (!isDone(state, "admin")) {
		const { adminUrl } = await deployAdmin({
			repoDir,
			workerUrl: requireState(state, "workerUrl"),
			apiKey: requireState(state, "apiKey"),
	...
	if (!isDone(state, "liff")) {
		const { liffUrl } = await deployLiff({
			repoDir,
			workerUrl: requireState(state, "workerUrl"),
			liffId: requireState(state, "liffId"),
	...
	if (addMcp && !p.isCancel(addMcp)) {
		generateMcpConfig({ workerUrl: requireState(state, "workerUrl"), apiKey: requireState(state, "apiKey") });
	}
	...
	p.note(
		[
			...
			`   ${pc.cyan(requireState(state, "liffUrl"))}`,
			...
			"",
			`${pc.bold("④ 管理画面:")}`,
			`   ${pc.cyan(requireState(state, "adminUrl"))}`,
			"",
			`${pc.bold("API Key:")}`,
			`   ${pc.dim(requireState(state, "apiKey"))}`,
