...
export async function runUpdate(repoDir: string): Promise<void> {
	...
	s.start("マイグレーション確認中...");
	try {
		await wrangler(["d1", "migrations", "apply", "line-harness", "--remote"], { cwd: join(repoDir, "packages/db") });
		s.stop("マイグレーション完了");
	} catch {
	...
	await wrangler(["pages", "deploy", "out", "--project-name", "line-harness-admin"], { cwd: webDir });
	...
	await wrangler(["pages", "deploy", "dist", "--project-name", "line-harness-liff"], { cwd: liffDir });
	s.stop("LIFF 再デプロイ完了");
