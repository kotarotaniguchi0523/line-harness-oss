...
export async function createTag(db: D1Database, input: CreateTagInput): Promise<Tag> {
	...
	const created = await db.prepare("SELECT * FROM tags WHERE id = ?").bind(id).first<Tag>();
	if (!created) throw new Error(`Failed to retrieve tag after insert: ${id}`);
	return created;
}
...
export async function addTagToFriend(db: D1Database, friendId: string, tagId: string): Promise<void> {
	const now = DateTime.now().toISO();
	await db
		.prepare(
