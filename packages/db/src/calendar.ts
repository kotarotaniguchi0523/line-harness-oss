...
export async function getCalendarConnections(db: D1Database): Promise<GoogleCalendarConnectionRow[]> {
	const result = await db
		.prepare("SELECT * FROM google_calendar_connections ORDER BY created_at DESC")
...
export async function createCalendarConnection(
	...
	await db
		...
		.run();
	const created = await getCalendarConnectionById(db, id);
	if (!created) throw new Error(`Failed to retrieve calendar connection after insert: ${id}`);
	return created;
}

/**
 * Update the OAuth tokens for a calendar connection.
 * Called after a successful Google OAuth token refresh to persist
 * the new access_token and its expiration timestamp in D1.
 */
export async function updateCalendarConnectionTokens(
	...
	await db
		.prepare(
			"UPDATE google_calendar_connections SET access_token = ?, token_expires_at = ?, updated_at = ? WHERE id = ?",
		)
		.bind(tokens.accessToken, tokens.tokenExpiresAt, DateTime.now().toISO(), id)
		.run();
}
...
export async function getCalendarBookings(
	...
	if (opts.friendId) {
		const result = await db
			.prepare("SELECT * FROM calendar_bookings WHERE friend_id = ? ORDER BY start_at ASC")
			.bind(opts.friendId)
	...
	if (opts.connectionId) {
		const result = await db
			.prepare("SELECT * FROM calendar_bookings WHERE connection_id = ? ORDER BY start_at ASC")
			.bind(opts.connectionId)
			.all<CalendarBookingRow>();
		return result.results;
	}
...
export async function createCalendarBooking(
	...
	await db
		...
		.run();
	const created = await getCalendarBookingById(db, id);
...
export async function updateCalendarBookingStatus(db: D1Database, id: string, status: string): Promise<void> {
	await db
		.prepare("UPDATE calendar_bookings SET status = ?, updated_at = ? WHERE id = ?")
		.bind(status, DateTime.now().toISO(), id)
		.run();
}

export async function updateCalendarBookingEventId(db: D1Database, id: string, eventId: string): Promise<void> {
	await db
		.prepare("UPDATE calendar_bookings SET event_id = ?, updated_at = ? WHERE id = ?")
		.bind(eventId, DateTime.now().toISO(), id)
		.run();
}
...
export async function getBookingsInRange(
	...
	const result = await db
		.prepare(
			`SELECT * FROM calendar_bookings WHERE connection_id = ? AND start_at >= ? AND end_at <= ? AND status != 'cancelled' ORDER BY start_at ASC`,
		)
