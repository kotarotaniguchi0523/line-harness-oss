...
export async function createForm(db: D1Database, input: CreateFormInput): Promise<Form> {
	...
	const created = await getFormById(db, id);
	if (!created) throw new Error(`Failed to retrieve form after insert: ${id}`);
	return created;
}
...
export async function updateForm(db: D1Database, id: string, input: UpdateFormInput): Promise<Form | null> {
...
export async function getFormSubmissions(db: D1Database, formId: string): Promise<FormSubmission[]> {
...
export async function createFormSubmission(db: D1Database, input: CreateFormSubmissionInput): Promise<FormSubmission> {
	...
	const created = await db.prepare("SELECT * FROM form_submissions WHERE id = ?").bind(id).first<FormSubmission>();
	if (!created) throw new Error(`Failed to retrieve form submission after insert: ${id}`);
	return created;
}
