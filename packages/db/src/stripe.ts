...
export async function getStripeEvents(
	db: D1Database,
	opts: { friendId?: string; eventType?: string; limit?: number } = {},
): Promise<StripeEventRow[]> {
	const limit = opts.limit ?? 100;
	if (opts.friendId) {
		const result = await db
			.prepare("SELECT * FROM stripe_events WHERE friend_id = ? ORDER BY processed_at DESC LIMIT ?")
			.bind(opts.friendId, limit)
			.all<StripeEventRow>();
		return result.results;
	}
	if (opts.eventType) {
		const result = await db
			.prepare("SELECT * FROM stripe_events WHERE event_type = ? ORDER BY processed_at DESC LIMIT ?")
			.bind(opts.eventType, limit)
			.all<StripeEventRow>();
		return result.results;
	}
	const result = await db
		.prepare("SELECT * FROM stripe_events ORDER BY processed_at DESC LIMIT ?")
		.bind(limit)
		.all<StripeEventRow>();
	return result.results;
}
...
export async function createStripeEvent(
	...
	await db
		...
		.run();
	const created = await db.prepare("SELECT * FROM stripe_events WHERE id = ?").bind(id).first<StripeEventRow>();
