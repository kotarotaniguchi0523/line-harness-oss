...
export async function createNotificationRule(
	...
	await db
		.prepare(
			"INSERT INTO notification_rules (id, name, event_type, conditions, channels, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
		)
		.bind(
			id,
			input.name,
		...
		.run();
	const created = await getNotificationRuleById(db, id);
...
export async function createNotification(
	...
	await db
		.prepare(
			"INSERT INTO notifications (id, rule_id, event_type, title, body, channel, metadata, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
		)
		.bind(
			id,
			input.ruleId ?? null,
		...
		.run();
	const created = await db.prepare("SELECT * FROM notifications WHERE id = ?").bind(id).first<NotificationRow>();
