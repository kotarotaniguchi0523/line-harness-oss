...
export async function getEntryRoutes(db: D1Database): Promise<EntryRoute[]> {
	const result = await db.prepare("SELECT * FROM entry_routes ORDER BY created_at DESC").all<EntryRoute>();
...
export async function createEntryRoute(db: D1Database, input: CreateEntryRouteInput): Promise<EntryRoute> {
	...
	const created = await db.prepare("SELECT * FROM entry_routes WHERE id = ?").bind(id).first<EntryRoute>();
...
export async function recordRefTracking(
	...
	await db
       ...
       (id, ref_code, friend_id, entry_route_id, source_url,
        fbclid, gclid, twclid, ttclid, yclid, utm_source, utm_medium, utm_campaign,
	...
	const created = await db.prepare("SELECT * FROM ref_tracking WHERE id = ?").bind(id).first<RefTracking>();
...
export async function getRefTrackingWithClickIds(db: D1Database, friendId: string): Promise<RefTracking | null> {
	return db
       ...
       WHERE friend_id = ?
       AND (fbclid IS NOT NULL OR gclid IS NOT NULL OR twclid IS NOT NULL OR ttclid IS NOT NULL OR yclid IS NOT NULL)
