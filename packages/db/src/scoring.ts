...
export async function createScoringRule(
	...
	await db
		...
		.run();
	const created = await getScoringRuleById(db, id);
	if (!created) throw new Error(`Failed to retrieve scoring rule after insert: ${id}`);
	return created;
}
...
export async function addScore(
	...
	await db
		.prepare(
			"INSERT INTO friend_scores (id, friend_id, scoring_rule_id, score_change, reason, created_at) VALUES (?, ?, ?, ?, ?, ?)",
		)
		.bind(id, input.friendId, input.scoringRuleId ?? null, input.scoreChange, input.reason ?? null, now)
		.run();

	// スコアキャッシュを更新
	await db
		.prepare("UPDATE friends SET score = score + ?, updated_at = ? WHERE id = ?")
		.bind(input.scoreChange, now, input.friendId)
		.run();
}
...
export async function applyScoring(db: D1Database, friendId: string, eventType: string): Promise<number> {
	...
	// Batch all score inserts and the cache update in a single transaction
	...
	// Update the cached score on friends table
	statements.push(
		db.prepare("UPDATE friends SET score = score + ?, updated_at = ? WHERE id = ?").bind(totalChange, now, friendId),
	);
