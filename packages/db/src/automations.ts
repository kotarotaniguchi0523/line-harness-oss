...
export async function getAutomations(db: D1Database): Promise<AutomationRow[]> {
	const result = await db
		.prepare(
            ...
            line_account_id, is_active, priority, created_at, updated_at
     FROM automations ORDER BY priority DESC, created_at DESC`,
		)
		.all<AutomationRow>();
...
export async function createAutomation(
	...
	await db
		...
		.run();
	const created = await getAutomationById(db, id);
	if (!created) throw new Error(`Failed to retrieve automation after insert: ${id}`);
	return created;
}
...
export async function getAutomationLogs(
	...
	if (automationId) {
		const result = await db
			.prepare(
				`SELECT id, automation_id, friend_id, event_data, actions_result, status, created_at
       FROM automation_logs WHERE automation_id = ? ORDER BY created_at DESC LIMIT ?`,
			)
...
export async function getActiveAutomationsByEvent(db: D1Database, eventType: string): Promise<AutomationRow[]> {
	const result = await db
		.prepare(
			`SELECT id, name, description, event_type, conditions, actions,
            line_account_id, is_active, priority, created_at, updated_at
     FROM automations WHERE event_type = ? AND is_active = 1 ORDER BY priority DESC`,
		)
		.bind(eventType)
		.all<AutomationRow>();
	return result.results;
