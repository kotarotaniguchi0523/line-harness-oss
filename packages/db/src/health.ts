...
export async function getAccountHealthLogs(
	db: D1Database,
	lineAccountId: string,
	limit = 50,
): Promise<AccountHealthLogRow[]> {
	const result = await db
		.prepare("SELECT * FROM account_health_logs WHERE line_account_id = ? ORDER BY created_at DESC LIMIT ?")
		.bind(lineAccountId, limit)
		.all<AccountHealthLogRow>();
...
export async function createAccountHealthLog(
	...
	await db
		.prepare(
			"INSERT INTO account_health_logs (id, line_account_id, error_code, error_count, check_period, risk_level, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
		)
		.bind(id, input.lineAccountId, input.errorCode ?? null, input.errorCount, input.checkPeriod, input.riskLevel, now)
		.run();
	const created = await db
		.prepare("SELECT * FROM account_health_logs WHERE id = ?")
		.bind(id)
		.first<AccountHealthLogRow>();
...
export async function getLatestRiskLevel(db: D1Database, lineAccountId: string): Promise<string> {
	const row = await db
		.prepare("SELECT risk_level FROM account_health_logs WHERE line_account_id = ? ORDER BY created_at DESC LIMIT 1")
		.bind(lineAccountId)
...
export async function getAccountMigrations(db: D1Database): Promise<AccountMigrationRow[]> {
	const result = await db
		.prepare("SELECT * FROM account_migrations ORDER BY created_at DESC")
		.all<AccountMigrationRow>();
	return result.results;
}
...
export async function createAccountMigration(
	...
	await db
		.prepare(
			"INSERT INTO account_migrations (id, from_account_id, to_account_id, total_count, created_at) VALUES (?, ?, ?, ?, ?)",
		)
		.bind(id, input.fromAccountId, input.toAccountId, input.totalCount, now)
		.run();
	const created = await getAccountMigrationById(db, id);
...
export async function updateAccountMigration(
	...
	if (updates.migratedCount !== undefined) {
	...
	values.push(id);
	await db
		.prepare(`UPDATE account_migrations SET ${sets.join(", ")} WHERE id = ?`)
		.bind(...values)
		.run();
}
