...
export async function createIncomingWebhook(
	...
	await db
		...
		.run();
	const created = await getIncomingWebhookById(db, id);
...
export async function createOutgoingWebhook(
	...
	await db
		...
		.run();
	const created = await getOutgoingWebhookById(db, id);
...
export async function getActiveOutgoingWebhooksByEvent(
	...
	const all = await db.prepare("SELECT * FROM outgoing_webhooks WHERE is_active = 1").all<OutgoingWebhookRow>();
