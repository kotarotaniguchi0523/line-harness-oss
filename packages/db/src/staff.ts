...
export async function getStaffByApiKey(db: D1Database, apiKey: string): Promise<StaffMember | null> {
	return db
...
export async function createStaffMember(db: D1Database, input: CreateStaffInput): Promise<StaffMember> {
	...
	const created = await db.prepare("SELECT * FROM staff_members WHERE id = ?").bind(id).first<StaffMember>();
	if (!created) throw new Error(`Failed to retrieve staff member after insert: ${id}`);
	return created;
}
