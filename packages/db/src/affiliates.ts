...
export async function createAffiliate(db: D1Database, input: CreateAffiliateInput): Promise<Affiliate> {
	...
	const created = await getAffiliateById(db, id);
...
export async function recordAffiliateClick(
	...
	const created = await db.prepare("SELECT * FROM affiliate_clicks WHERE id = ?").bind(id).first<AffiliateClick>();
