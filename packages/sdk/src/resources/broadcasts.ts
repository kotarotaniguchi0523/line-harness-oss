...
import type { ApiResponse, Broadcast, CreateBroadcastInput, SegmentCondition, UpdateBroadcastInput } from "../types.js";
...
export class BroadcastsResource {
	...
	async sendToSegment(id: string, conditions: SegmentCondition): Promise<Broadcast> {
		const res = await this.http.post<ApiResponse<Broadcast>>(`/api/broadcasts/${id}/send-segment`, { conditions });
		return res.data;
	}
}
